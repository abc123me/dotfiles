...
if [ -z "$TMUX_POWERLINE_LEFT_STATUS_SEGMENTS" ]; then
	TMUX_POWERLINE_LEFT_STATUS_SEGMENTS=(
		...
		"lan_ip 14 0 ${TMUX_POWERLINE_SEPARATOR_RIGHT}"
		"vcs_branch 78 0"
		"pwd 208 0"
	)
fi
...
if [ -z "$TMUX_POWERLINE_RIGHT_STATUS_SEGMENTS" ]; then
	...
	if [ -d "/sys/class/power_supply/BAT0" ]; then
		TMUX_POWERLINE_RIGHT_STATUS_SEGMENTS+=("battery 208 82")
	fi
	TMUX_POWERLINE_RIGHT_STATUS_SEGMENTS+=(
		"weather 94 15"
		"$(
			if (($(tp_cpu_temp_at_least 80))); then
				echo "cpu_temp 196 15"
			elif (($(tp_cpu_temp_at_least 60))); then
				echo "cpu_temp 202 15"
			else
				echo "cpu_temp 33 15"
			fi
		)"
		"date_day 99 15"
		"date     99 15 ${TMUX_POWERLINE_SEPARATOR_LEFT_THIN}"
		"time     99 15 ${TMUX_POWERLINE_SEPARATOR_LEFT_THIN}"
	)
fi
